...
import numpy as np
from data import Rna8fSubsetsDataset
...
train_dataset = Rna8fSubsetsDataset(
...
test_sample_path_list = [
    os.path.join(dataset_root_path, sub_dataset, 'test') for sub_dataset in sub_dataset_list]
...
def train_valid_test():
    ...
    print(f"Model with best valid F1 score is saved to: {os.path.join(result_path, f'model_state_dict.pt')}")
